...
export const validate = (value) => !!value.length;
/**
 * Phone formatter.
...
export const format = (value, opts) => {
    ...
    const { strict, areaCode, countryCode, countryCodeLength, areaCodeLength, phoneNumberLength, phoneNumberDelimiter, blocksDelimiter, countryCodePrefix, areaBrackets, mask } = Object.assign({}, DEFAULT_OPTS, opts);
...
export const parseBlocks = (value, ...blocks) => {
...
export const resolveBlockLengths = (entireLength, ...blocks) => {
    const known = blocks.filter(v => v !== undefined);
...
export const formatAreaCode = (value, brackets) => {
...
export const formatCountryCode = (value, prefix) => {
    if (!value || !value.length) {
        return null;
    }
    return prefix + value;
