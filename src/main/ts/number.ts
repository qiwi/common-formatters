 ...
 */
export const formatNumber: IFormatter = (
  value: IAny,
  opts?: IFormatNumberOpts,
): IFormatted => {
  const {
    fractionDelimiter,
    fractionLength,
  ...
  const _value =
    fl === undefined ? Math.abs(num).toString() : Math.abs(num).toFixed(fl)

  const signPrefix = num < 0 ? MINUS_SIGN : sign && num > 0 ? PLUS_SIGN : ''
